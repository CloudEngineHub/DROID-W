...
def get_feature_extractor(cfg: Dict) -> nn.Module:
    ...
    elif extractor_model in ["dinov3_vits16"]:
        REPO_DIR = "facebookresearch/dinov3"
        dinov3_vits16 = torch.hub.load(
            REPO_DIR, 
            extractor_model, 
            source="github",
            weights="dinov3_pth/dinov3_vits16_pretrain_lvd1689m-08c60483.pth",
        )
        return dinov3_vits16.to(device).eval()
    elif extractor_model in ["dinov3_vits16plus"]:
        REPO_DIR = "facebookresearch/dinov3"
        dinov3_vits16plus = torch.hub.load(
            REPO_DIR, 
            extractor_model, 
            source="github",
            weights="dinov3_pth/dinov3_vits16plus_pretrain_lvd1689m-4057cbaa.pth"
        )
        return dinov3_vits16plus.to(device).eval()
    else:
...
def _save_features(features: torch.Tensor, cfg: Dict, idx: int, suffix: str) -> None:
    ...
    os.makedirs(f"{output_dir}/mono_priors/features", exist_ok=True)
