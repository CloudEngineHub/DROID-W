...
class PointCloudViewer:
    def __init__(
        ...
        self.depth_filter_threshold = 0.05
        self.depth_grad_threshold = 0.002
        ...
        self.poses = poses
        self.disps = disps
        self.intr_iproj = intr_iproj
        self.uncertainties = uncertainties
        self.depth_grad_norm = depth_grad_norm
        ...
        self.start_frame = 0
        self.dynamic_pts_downsample_factor = 1
        ...
        button5 = self.server.gui.add_button("Only Show Current Camera")

        self.is_render = False
        self.fourd = False
        self.only_cur_cam = False

        @button3.on_click
        def _(event: viser.GuiEvent) -> None:
            self.fourd = True
            self.only_cur_cam = True
        ...
        self.static_threshold_slider = self.server.add_gui_slider(
            "Static Threshold",
            min=0.0,
            max=1.0,
            step=0.05,
            initial_value=self.static_threshold,
        )

        self.dynamic_threshold_slider = self.server.add_gui_slider(
            "Dynamic Threshold",
            min=0.8,
            max=2.0,
            step=0.05,
            initial_value=self.dynamic_threshold,
        )

        self.filter_threshold_slider = self.server.add_gui_slider(
            "Filter Threshold",
            min=0.001,
            max=0.5,
            step=0.001,
            initial_value=self.filter_threshold,
        )

        self.depth_filter_threshold_slider = self.server.add_gui_slider(
            "Depth Filter Threshold",
            min=0.01,
            max=0.5,
            step=0.05,
            initial_value=self.depth_filter_threshold,
        )

        self.depth_grad_threshold_slider = self.server.add_gui_slider(
            "Depth Grad Threshold",
            min=0.002,
            max=0.2,
            step=0.005,
            initial_value=self.depth_grad_threshold,
        )
        ...
        self.dynamic_pts_downsample_slider = self.server.add_gui_slider(
            "Dynamic Points Downsample Factor",
            min=1,
            max=50,
            step=1,
            initial_value=1,
        ...
        self.frame_downsample_slider = self.server.add_gui_slider(
            "Frame Downsample Factor",
            min=1,
            max=50,
            step=1,
            initial_value=1,
        ...
        self.start_frame_slider = self.server.add_gui_slider(
            "Start Frame",
            min=0,
            max=self.num_frames - 1,
            step=1,
            initial_value=self.start_frame,
        )

        self.pc_handles = []
        self.cam_handles = []

        @self.start_frame_slider.on_update
        def _(_) -> None:
            self.start_frame = self.start_frame_slider.value
            if hasattr(self, 'frame_nodes'):
                ...
                for i, step in enumerate(self.all_steps):
                    if step < self.start_frame:
                        continue
                    pc = self.pcs[step]["pc"]
                    color = self.pcs[step]["color"]
                    conf = self.pcs[step]["conf"]
                    edge_color = self.pcs[step].get("edge_color", None)
                    uncertainty = self.uncertainties[step]
                    depth_grad_norm = self.depth_grad_norm[step]
                    disps = self.disps[step]
                    cur_dynamic_pts_active = True if (step % self.dynamic_pts_downsample_factor == 0) else False
                    
                    pred_pts, color_pts = self.parse_pc_data(
                        pc, color, conf, uncertainty, depth_grad_norm, disps, edge_color, set_border_color=self.set_border_color, 
                        downsample_factor=self.downsample_slider.value,
                        cur_dynamic_pts_active=cur_dynamic_pts_active
                    )
                ...
                if hasattr(self, 'frame_nodes'):
                    ...
                    if self.show_camera:
                        downsample_factor = int(self.frame_downsample_slider.value)
                        for i, step in enumerate(self.all_steps):
                            if step < self.start_frame:
                                continue
                            else:
        ...
        def _(_) -> None:
            self.point_size = self.psize_slider.value
            for handle in self.pc_handles:
        ...
        def _(_) -> None:
            for handle in self.cam_handles:
                ...
                handle.line_thickness = 0.1 * handle.scale
        ...
        @self.static_threshold_slider.on_update
        def _(_) -> None:
            # when the visualization threshold changes, update the threshold and regenerate the point cloud
            self.static_threshold = self.static_threshold_slider.value
            if hasattr(self, 'frame_nodes'):
                # safely clear the existing point cloud display
                for handle in self.pc_handles:
                    try:
        ...
        @self.dynamic_threshold_slider.on_update
        def _(_) -> None:
            # when the visualization threshold changes, update the threshold and regenerate the point cloud
            self.dynamic_threshold = self.dynamic_threshold_slider.value
            if hasattr(self, 'frame_nodes'):
                # safely clear the existing point cloud display
                for handle in self.pc_handles:
                    try:
        ...
        @self.depth_grad_threshold_slider.on_update
        def _(_) -> None:
            # when the visualization threshold changes, update the threshold and regenerate the point cloud
            self.depth_grad_threshold = self.depth_grad_threshold_slider.value
            if hasattr(self, 'frame_nodes'):
                # safely clear the existing point cloud display
                for handle in self.pc_handles:
                    try:
        ...
        @self.filter_threshold_slider.on_update
        def _(_) -> None:
            # when the visualization threshold changes, update the threshold and regenerate the point cloud
            self.filter_threshold = self.filter_threshold_slider.value
            if hasattr(self, 'frame_nodes'):
                # safely clear the existing point cloud display
                for handle in self.pc_handles:
                    try:
                ...
                counts = droid_backends.depth_filter(
                    ...
                    self.filter_threshold * torch.ones_like(self.disps.mean(dim=[1, 2]))
                )
                ...
                for i, step in enumerate(self.all_steps):
                    ...
                    self.pcs[step]["conf"] = conf_list[i].cpu().numpy()
                    conf = self.pcs[step]["conf"]
                    edge_color = self.pcs[step].get("edge_color", None)
                    uncertainty = self.uncertainties[step]
                    depth_grad_norm = self.depth_grad_norm[step]
        ...
        @self.depth_filter_threshold_slider.on_update
        def _(_) -> None:
            # when the visualization threshold changes, update the threshold and regenerate the point cloud
            self.depth_filter_threshold = self.depth_filter_threshold_slider.value
            if hasattr(self, 'frame_nodes'):
                # safely clear the existing point cloud display
                for handle in self.pc_handles:
                    try:
        ...
        @self.frame_downsample_slider.on_update
        def _(_) -> None:
            # when the frame downsample factor changes, update the frame display
            self.all_steps = [i for i in range(0, self.num_frames, int(self.frame_downsample_slider.value))]
            if hasattr(self, 'frame_nodes'):
                # clear the existing camera display
                for handle in self.cam_handles:
                    try:
        ...
        @self.dynamic_pts_downsample_slider.on_update
        def _(_) -> None:
            self.dynamic_pts_downsample_factor = self.dynamic_pts_downsample_slider.value
    ...
    def _connect_client(self, client: viser.ClientHandle):
        ...
        #             pts3ds, colors, set_border_color=self.set_border_color, 
        #             downsample_factor=self.downsample_slider.value
        #         )
    ...
    def read_data(self, pc_list, color_list, conf_list, edge_color_list=None):
        ...
        self.camera_colors = edge_color_list
    ...
    def parse_pc_data(
        ...
        if conf is not None:
            ...
            depth_grad_norm = depth_grad_norm.reshape(-1)
            disps = disps.reshape(-1).cpu().numpy()
            pred_pts = pred_pts[(((conf >= self.vis_threshold) \
                & (uncertainty < self.static_threshold)) \
                | ((uncertainty > self.dynamic_threshold) & cur_dynamic_pts_active)) \
                & (depth_grad_norm < self.depth_grad_threshold) \
                & (disps > self.depth_filter_threshold)]
            color = color[(((conf >= self.vis_threshold) \
                & (uncertainty < self.static_threshold)) \
                | ((uncertainty > self.dynamic_threshold) & cur_dynamic_pts_active)) \
                & (depth_grad_norm < self.depth_grad_threshold) \
                & (disps > self.depth_filter_threshold)]
                # keep static + find dynamic + filter depth and depth gradient
    ...
    def add_camera(self, step):
        ...
        camera_color = self.camera_colors[step]
        ...
        self.cam_handles.append(
            self.server.add_camera_frustum(
                ...
                # image=frame_rgb[::self.downsample_factor, ::self.downsample_factor],
    ...
    def animate(self):
        ...
        def _(_) -> None:
            gui_timestep.value = (gui_timestep.value + self.frame_downsample_slider.value) % self.num_frames
        ...
        while True:
            if self.on_replay:
                ...
                for i, frame_node in enumerate(self.frame_nodes):
                    ...
                    #     self.cam_handles[i].visible = (
                    #         i <= gui_timestep.value
                    #         if not self.only_cur_cam
                    #         else i == gui_timestep.value
                    #     )
